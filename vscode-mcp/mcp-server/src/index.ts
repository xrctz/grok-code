...
server.tool(
  'vscode_ping',
  'Check whether the Grok Code bridge is reachable and report status.',
  {},
  async () => {
    try {
...
server.tool(
  'vscode_status',
  'Get live Grok Code status: app name, workspace folders, active file, dirty editors.',
  {},
  async () => {
    try {
...
server.tool(
  'vscode_workspace',
  'List workspace folders currently open in VS Code.',
  {},
  async () => {
    try {
...
server.tool(
  'vscode_list_editors',
  'List active and visible text editors plus open tab count.',
  {},
  async () => {
    try {
...
server.tool(
  ...
  'Read the current selection (or cursor position) in the active editor.',
  {},
  async () => {
    try {
...
server.tool(
  'vscode_get_document',
  'Read the full text of a document. Omit path to use the active editor.',
  {
    path: z
      .string()
      .optional()
...
server.tool(
  ...
  'Get problems/diagnostics (errors, warnings) from VS Code. Optionally filter by path.',
  {
...
server.tool(
  'vscode_search_files',
  'Search workspace files by glob or substring via VS Code findFiles.',
  {
    query: z
      .string()
...
server.tool(
  'vscode_search_text',
  'Search file contents across the workspace (substring). Returns path, line, column, preview.',
  {
  ...
  async ({ query, include, maxResults, caseSensitive }) => {
    try {
      return textResult(
        await bridge.post('/search-text', { query, include, maxResults, caseSensitive })
      );
    } catch (err) {
      return errorResult(err);
    }
...
server.tool(
  'vscode_open_file',
  'Open a file in VS Code and optionally jump to a line/column (1-based).',
  {
...
server.tool(
  'vscode_insert_text',
  'Insert text at the cursor in the active editor (or a given path).',
  {
...
server.tool(
  'vscode_replace_selection',
  'Replace the current selection with new text (or insert at cursor if empty selection).',
  {
...
server.tool(
  'vscode_edit',
  'Apply one or more range edits to a file (1-based line/column). Optionally save.',
  {
...
server.tool(
  'vscode_save',
  'Save the active document, a specific path, or all dirty editors.',
  {
...
server.tool(
  'vscode_run_command',
  'Execute a VS Code command by id (e.g. workbench.action.files.save, editor.action.formatDocument).',
  {
...
server.tool(
  'vscode_show_message',
  'Show a short message in Grok Code. Info goes to the status bar by default (avoids pausing the built-in Browser). Set toast=true only if you need a real notification.',
  {
...
server.tool(
  'vscode_tabs',
  'List all open editor tabs including Grok Code Browser / webview tabs (not just text editors).',
  {},
  async () => {
    try {
...
server.tool(
  'vscode_browser',
  'Inspect whether the Grok Code built-in Browser is open and which browser tabs are active.',
  {},
  async () => {
    try {
...
server.tool(
  'vscode_browser_open',
  'Open a URL in the Grok Code built-in Browser (not an external Chrome window) and clear pausing notifications.',
  {
...
server.tool(
  'vscode_notifications_clear',
  'Hide/clear notification toasts so the Grok Code Browser is not paused ("Paused due to Notification").',
  {},
  async () => {
    try {
...
server.tool(
  ...
  'Capture what is on the Grok Code Browser right now. Returns a file path + base64 image and HUD/meta when the page streams frames (e.g. 3D Zombie Game). Falls back to headless Chrome of the page URL.',
  {
    force: z
      .boolean()
      .optional()
  ...
  async ({ force, maxAgeMs, url }) => {
    try {
      return textResult(
        await bridge.post('/browser/screenshot', { force: !!force, maxAgeMs, url })
      );
    } catch (err) {
      return errorResult(err);
    }
...
server.tool(
  'vscode_reveal_line',
  'Jump the cursor to a line/column and reveal it in the editor.',
  {
...
server.tool(
  ...
  'Create and open a new VS Code integrated terminal.',
  {
...
server.tool(
  'vscode_terminal_send_text',
  'Send command text / run command in a VS Code integrated terminal.',
  {
...
server.tool(
  ...
  'List all open VS Code integrated terminals.',
  {},
  async () => {
    try {
...
server.tool(
  'vscode_terminal_close',
  'Close/dispose a specific VS Code integrated terminal by name.',
  {
...
server.tool(
  'vscode_terminal_output',
  'Read recently captured integrated-terminal output (best-effort). Prefer vscode_shell_exec for reliable stdout/exit codes.',
  {
...
server.tool(
  'vscode_shell_exec',
  'Run a shell command with captured stdout, stderr, and exit code. Preferred for agent coding loops (tests, builds, git status). Optionally mirrors into the integrated terminal.',
  {
...
server.resource(
  'vscode-status',
  'vscode://status',
  { description: 'Live VS Code bridge status snapshot', mimeType: 'application/json' },
  async () => {
    try {
...
async function main(): Promise<void> {
  ...
  console.error(
    `[grok-code] ready → bridge ${config.baseUrl} (token ${config.token ? 'set' : 'MISSING'})`
  );
}
