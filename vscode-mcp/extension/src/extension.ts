...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('vscodeMcpBridge.start', () => startBridge(context)),
    vscode.commands.registerCommand('vscodeMcpBridge.stop', () => stopBridge()),
    vscode.commands.registerCommand('vscodeMcpBridge.copyToken', async () => {
      if (!sessionToken) {
    ...
    vscode.commands.registerCommand('vscodeMcpBridge.showStatus', () => {
      ...
      vscode.window
        .showInformationMessage(
          `Grok Code bridge listening on http://${sessionHost}:${sessionPort}`,
          'Copy Token',
          'Copy URL'
        )
        .then(async (choice) => {
...
async function startBridge(context: vscode.ExtensionContext): Promise<void> {
  ...
  const port = cfg.get<number>('port', 7331);
  const host = cfg.get<string>('host', '127.0.0.1');
  const configuredToken = cfg.get<string>('token', '')?.trim();
  sessionToken =
    configuredToken ||
    (context.workspaceState.get<string>('mcpBridge.sessionToken') ??
  ...
  bridge = new BridgeServer({ host, port, token: sessionToken });
  ...
  try {
    ...
    if (cfg.get<boolean>('autoRegisterMcp', false)) {
    ...
    vscode.window.setStatusBarMessage(
      `Grok Code bridge on http://${host}:${port}`,
      5000
    );
  } catch (err) {
...
async function quietNotifications(): Promise<void> {
  for (const cmd of [
...
function envBody(host: string, port: number, token: string): string {
  return [
...
async function writeClientEnvFiles(
  host: string,
  port: number,
  token: string
): Promise<void> {
...
function resolveMcpServerEntry(context: vscode.ExtensionContext): string | undefined {
  const cfg = vscode.workspace.getConfiguration('vscodeMcpBridge');
  const configured = cfg.get<string>('mcpServerPath', '')?.trim();
...
async function autoRegisterMcpClients(
...
async function mergeJsonMcpServer(
  configPath: string,
  name: string,
  serverBlock: { command: string; args: string[]; env: Record<string, string> }
): Promise<void> {
  ...
  try {
    ...
    (config.mcpServers as Record<string, unknown>)[name] = serverBlock;
...
`;
    ...
    if (/\[mcp_servers\.grok-code\]/.test(text)) {
      ...
      text = text.replace(
        /\[mcp_servers\.grok-code\][\s\S]*?(?=\n\[(?!mcp_servers\.grok-code\.env)|$)/,
        ''
      );
      text = text.replace(
        /\[mcp_servers\.grok-code\.env\][\s\S]*?(?=\n\[|$)/,
        ''
      );
      text = text.trimEnd() + '\n\n' + section;
    } else {
