...
class BridgeServer {
    ...
    terminalDataHooked = false;
    static TERMINAL_BUF_MAX = 64 * 1024;
    constructor(options) {
    ...
    async handle(req, res) {
        ...
        if (pathname === '/browser/frame' && req.method === 'POST') {
            ...
            try {
                ...
                return this.send(res, 200, await this.acceptBrowserFrame(json));
            }
            catch (err) {
        ...
        try {
            const body = req.method === 'POST' || req.method === 'PUT' ? await readBody(req, MAX_BODY_BYTES) : undefined;
            const json = body ? safeJson(body) : undefined;
            switch (`${req.method} ${pathname}`) {
                ...
                case 'GET /search-text':
                    return this.send(res, 200, await this.searchText({
                ...
                case 'POST /search-text':
                    return this.send(res, 200, await this.searchText(json));
                case 'GET /tabs':
                ...
                case 'POST /open':
                    return this.send(res, 200, await this.openFile(json));
                case 'POST /insert':
                    return this.send(res, 200, await this.insertText(json));
                case 'POST /replace-selection':
                    return this.send(res, 200, await this.replaceSelection(json));
                case 'POST /edit':
                    return this.send(res, 200, await this.applyEdit(json));
                case 'POST /save':
                    return this.send(res, 200, await this.save(json));
                case 'POST /command':
                    return this.send(res, 200, await this.runCommand(json));
                case 'POST /show-message':
                    return this.send(res, 200, await this.showMessage(json));
                case 'POST /reveal-line':
                    return this.send(res, 200, await this.revealLine(json));
                case 'POST /browser/open':
                    return this.send(res, 200, await this.openBrowser(json));
                case 'POST /browser/screenshot':
                    return this.send(res, 200, await this.getBrowserScreenshot(undefined, json));
                case 'POST /notifications/clear':
                    return this.send(res, 200, await this.clearNotifications());
                case 'POST /create-file':
                    return this.send(res, 200, await this.createFile(json));
                case 'POST /delete-file':
                    return this.send(res, 200, await this.deleteFile(json));
                case 'POST /rename-file':
                    return this.send(res, 200, await this.renameFile(json));
                case 'POST /terminal/create':
                    return this.send(res, 200, await this.createTerminal(json));
                case 'POST /terminal/send-text':
                    return this.send(res, 200, await this.sendTerminalText(json));
                case 'GET /terminal/list':
                    return this.send(res, 200, await this.listTerminals());
                case 'POST /terminal/close':
                    return this.send(res, 200, await this.closeTerminal(json));
                case 'GET /terminal/output':
                    return this.send(res, 200, await this.getTerminalOutput({
                        name: url.searchParams.get('name') || undefined,
                        maxChars: Number(url.searchParams.get('maxChars') || '8000')
                    }));
                case 'POST /terminal/output':
                    return this.send(res, 200, await this.getTerminalOutput(json));
                case 'POST /shell/exec':
                    return this.send(res, 200, await this.shellExec(json));
                default:
    ...
    authorized(req) {
        ...
        const bearer = typeof header === 'string' && header.startsWith('Bearer ')
            ? header.slice(7).trim()
            : '';
        const xToken = req.headers['x-mcp-token']?.trim() || '';
    ...
    getBrowser() {
        const tabs = vscode.window.tabGroups.all.flatMap((g) => g.tabs.map((t) => serializeTab(t)));
        ...
        return {
            ...
            frame: this.latestFrame
                ...
                : null,
            note: 'Use GET /browser/screenshot for a live image (game posts canvas frames; Chrome headless is the fallback).'
        };
    }
    frameDir() {
    ...
    async acceptBrowserFrame(payload) {
    ...
    async getBrowserScreenshot(query, body) {
        ...
        const url = (body?.url || '').trim() ||
            query?.get('url') ||
            guessBrowserUrl(this.getBrowser()) ||
            'http://127.0.0.1:8765/';
    ...
    async replaceSelection(payload) {
        if (payload?.text == null) {
            throw new Error('text is required');
        }
        const editor = await ensureEditor(payload.path);
    ...
    async applyEdit(payload) {
        ...
        for (const e of payload.edits) {
            ...
            const end = new vscode.Position(Math.max(0, (e.endLine ?? e.startLine ?? 1) - 1), Math.max(0, (e.endColumn ?? e.startColumn ?? 1) - 1));
    ...
    async showMessage(payload) {
        ...
        const wantToast = payload.toast === true || (payload.toast !== false && type === 'error');
    ...
    async getTerminalOutput(payload) {
...
function captureUrlWithChrome(url, dir) {
    return new Promise((resolve, reject) => {
        ...
        const child = (0, child_process_1.spawn)(chrome, args, { stdio: ['ignore', 'ignore', 'pipe'] });
        let err = '';
        child.stderr.on('data', (d) => {
            err += d.toString();
        });
...
function classifyTabKind(label, inputType, uri) {
    const blob = `${label} ${inputType} ${uri || ''}`.toLowerCase();
    if (blob.includes('browser') ||
...
async function resolveDocument(filePath) {
    if (filePath) {
...
async function ensureEditor(filePath) {
    if (filePath) {
        const uri = toUri(filePath);
        const doc = await vscode.workspace.openTextDocument(uri);
